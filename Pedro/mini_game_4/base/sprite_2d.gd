...
#func borrar_en_posicion(pos_local: Vector2):
	...
	#var modificado = false
	#
	## Cambia a transparente los píxeles dentro del radio del círculo
	#for x in range(centro_x - radio_borrado, centro_x + radio_borrado):
		#for y in range(centro_y - radio_borrado, centro_y + radio_borrado):
			## Verifica que el píxel esté dentro de los límites de la imagen
			#if x >= 0 and x < imagen_editable.get_width() and y >= 0 and y < imagen_editable.get_height():
				## Ecuación del círculo para un borrado redondo y suave
				#if Vector2(x, y).distance_to(Vector2(centro_x, centro_y)) < radio_borrado:
					#imagen_editable.set_pixel(x, y, Color(0, 0, 0, 0)) # Píxel transparente
					#modificado = true
	#
	## Si la imagen cambió, actualizamos la textura en pantalla
	#if modificado:
		#textura_dinamica.update(imagen_editable)
